...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"toArtistView"])
    {
        // Fetch the selected object
        NSManagedObject *o = [self.model.frc_artist objectAtIndexPath:sender];
        //Configure the next VC
        ArtistView *vc = (ArtistView *)segue.destinationViewController;
        vc.model = self.model;
        vc.o = o;
        vc.title = [o valueForKey:@"artistName"];
    }
}

- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    [self performSegueWithIdentifier:@"toArtistView" sender:indexPath];
}
