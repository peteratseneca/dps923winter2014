...
// Before using an frc, you must send it the performFetch message
...
- (NSFetchedResultsController *)frcWithEntityNamed:(NSString *)entityName withPredicateFormat:(NSString *)predicate predicateObject:(id)predicateObject sortDescriptors:(NSString *)sortDescriptors andSectionNameKeyPath:(NSString *)sectionName
{
    ...
    // Therefore, if your app is complex and you need the cache, replace "nil" with "entityName"
    
    return [[NSFetchedResultsController alloc] initWithFetchRequest:fetchRequest managedObjectContext:self.managedObjectContext sectionNameKeyPath:sectionName cacheName:nil];
}
