    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        else
        {
            ...
            else
            {
                ...
                // New code added...
                // Go through the fetched results, and create locally-stored objects

                for (NSDictionary *d in _programs)
                {
                    Program *p = (Program *)[self addNew:@"Program"];
                    p.objectIdentifier = d[@"Id"];
                    p.code = d[@"Code"];
                    p.name = d[@"Name"];
                    p.objectDescription = d[@"Description"];
                    p.credential = d[@"Credential"];
                    p.semesters = d[@"Semesters"];
                }
                [self saveChanges];
            }
...
- (NSFetchedResultsController *)frc_program
{
    ...
    if (_frc_program) { return _frc_program; }
    
    // Otherwise, create a new frc, and set it as the property (and return it below)
    _frc_program = [_cdStack frcWithEntityNamed:@"Program" withPredicateFormat:nil predicateObject:nil sortDescriptors:@"code,YES" andSectionNameKeyPath:nil];
